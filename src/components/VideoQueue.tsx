...
import type { VideoItem, VideoConfig } from '../types';
import { formatBytes, formatDimensions, formatDuration } from '../utils/formatters';
...
const VideoRow: React.FC<{
  ...
  useEffect(() => {
    ...
    if (!thumb && window.electronAPI) {
      window.electronAPI.getVideoThumbnail(item.path).then((data) => {
  ...
  const targetWidthDisplay =
    ...
    item.originalWidth &&
    item.originalWidth <= config.maxWidth
      ? item.originalWidth
      : config.maxWidth;
  ...
  return (
    <div className="flex items-center gap-3 p-2.5 bg-white dark:bg-[#182035] hover:bg-slate-50 dark:hover:bg-[#1e2740] border border-slate-200 dark:border-[#283254] rounded-xl transition-all group shadow-sm">
      {/* Thumbnail */}
      <div className="w-11 h-11 rounded-lg bg-slate-100 dark:bg-[#121625] border border-slate-200 dark:border-[#263150] overflow-hidden flex-shrink-0 flex items-center justify-center relative">
        {thumb ? (
          <img
            src={thumb}
            alt={item.name}
          ...
          <FileVideo className="w-5 h-5 text-slate-400 dark:text-slate-500" />
        )}
      </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-0.5 text-[11px] text-slate-500 dark:text-darkTextMuted">
          ...
          {item.status === 'done' ? (
            ...
            <span className="text-slate-700 dark:text-slate-300">
              max {targetWidthDisplay}px → {config.outputFormat.toUpperCase()}
            </span>
          )}
        </div>
        ...
        {item.status === 'processing' && (
          <div className="mt-1.5 h-1 rounded-full bg-slate-200 dark:bg-[#263150] overflow-hidden">
            <div
